  ...
  const Products = () => {
    ...
    const [categories, setCategories] = useState(["all"]); // 👈 LOWERCASE
    const [loading, setLoading] = useState(true);

    const [selectedCategory, setSelectedCategory] = useState("all"); // 👈 LOWERCASE
    ...
    useEffect(() => {
      const fetchProducts = async () => {
        try {
          ...
          // Categories mapped to lowercase 🔥
          const uniqueCats = [
            "all",
            ...new Set(
              data.map((p: any) => p.category?.toLowerCase())
            ),
          ];
